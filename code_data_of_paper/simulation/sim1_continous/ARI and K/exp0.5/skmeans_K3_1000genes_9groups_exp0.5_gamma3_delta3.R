...
save(res.skm,gen.res,file="skmeans_K3_1000genes_9groups_exp0.5_gamma3_delta3.rdata")
...
FPs<-c()
for(i in 1:100){
  TPs[i]<-sum(which(abs(res.skm[[i]][[3]])>0) %in% 1:15)
  TNs[i]<-sum(which(abs(res.skm[[i]][[3]])==0) %in% 16:1000)
  FNs[i]<-sum(which(abs(res.skm[[i]][[3]])==0) %in% 1:15)
  FPs[i]<-sum(which(abs(res.skm[[i]][[3]])>0) %in% 16:1000)
}
