...
gen.train<-gen.data.K3(n.rep, NG, n, theta, cluster_index, exp=1)
data.train<-gen.train$data

gen.test<-gen.data.K3(n.rep, NG, n, theta, cluster_index, exp=1)
...
par.est.all <- foreach(rep = 1:n.rep, .packages = c("mclust","OGClust")) %dopar% {
  ...
  for (l in 1:nrow(tune_pars)) {
    ...
    gamma_est_matrix <- matrix(res.all[[l]][3:((K - 1) * (NG + 1) + 2)], ncol = K - 1, byrow = T)
    ng_select<-sum(apply(gamma_est_matrix[-1,,drop=F], 1, function(x) sum(abs(x)>0))>0)
    if (ng_select==0) BIC.all[l]<-Inf
  }
...
save(par.est.all, gen.train, gen.test, file = "ogclust_K3_1000genes_9groups_exp1_gamma1_delta3.rdata")
...
ARI 
#[1] 0.8936914 0.8608608 0.8456074 0.8821374 0.8523640 0.8904011 0.8738973 0.8554322 0.8837425 0.8817019
mean(ARI) #[1] 0.8777996
