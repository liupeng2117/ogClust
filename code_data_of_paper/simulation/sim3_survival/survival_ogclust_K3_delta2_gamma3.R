...
# 
#---------------------------------------------------#
par.est.all <- foreach(rep = 1:n.rep, .packages = c("mclust")) %dopar% {
  # print(rep)
  df <- data.train[[rep]]
  df=df[,-c((dim(df)[2]-3):dim(df)[2])]
  X=as.matrix(df[,1:np])
  df[, (np+1):(np+NG)] <- standardize(as.matrix(df[, (np+1):(np+NG)])) # standardize
  G=as.matrix(df[,(np+1):(np+NG)])
  Y <- df$time
  delta=df$event
  
  df_test <- data.test[[rep]]
  df_test=df_test[,-c((dim(df_test)[2]-3):dim(df_test)[2])]
  X_test=as.matrix(df_test[,1:np])
  df_test[, (np+1):(np+NG)] <- standardize(as.matrix(df_test[, (np+1):(np+NG)])) # standardize
  G_test <- as.matrix(df_test[, (np+1):(np+NG)])
  Y_test <- df_test$time
  delta_test=df_test$event
  ...
  for (l in 1:nrow(tune_pars)) {
    ...
    for (i in 1:n.int) {
      ...
      beta0_int <- runif(K,0,2)
      sigma2_int <- runif(1,0,2)
      theta_int <- c(beta_int, gamma_int, beta0_int, sigma2_int)
      fit.res=fit.OGClust.surv(n, K, np, NG, lambda, alpha=alpha, Y=Y, G=G, X=X, delta=delta, theta_int)
    ...
    beta0_est <- best.par[((K - 1) * (NG + 1) + 3):((K - 1) * (NG + 1) + 2 + K)]
    ...
    #f_est <- sapply(1:K, function(x) (1 / sqrt(2 * pi * sigma2_est)) * exp(-(Y - beta0_est[x] - X1 * beta1_est - X2 * beta2_est)^2 / (2 * sigma2_est)))
    ...
    Y_prd_train <- exp(beta0_est[grp_assign_train] + X %*% beta_est)
    Y_prd_train <- ifelse(Y_prd_train>100,100,Y_prd_train)
    ...
    f_est=sapply(1:K, function(x) f_calc(Y1=Y_test,X1=X_test,beta = beta_est,mu=beta0_est[x],sigma2 = sigma2_est,delta = delta))
    f_est=t(apply(f_est,1,function(x) x/sum(x)))
    idx=which.max(beta0_est)
    test=apply(f_est,1,sum)
    ...
    grp_assign_test<-apply(w_est_test,1,which.max)
    (ARI_test <- adjustedRandIndex(grp_assign_test, gen.test$grp_assign[rep, ]))
    Y_prd_test <- exp(beta0_est[grp_assign_test] + X_test %*% beta_est)
...
save(par.est.all, gen.train, gen.test, file = "survival_ogclust_K3_delta2_gamma3.rdata")
