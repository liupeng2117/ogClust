...
# ------- nonrobust --------
#-------------------------------
load(file = "ogclust_K3_1000genes_6groups_gamma1_delta3_outlier_none.rdata")
#---------------------
...
res.none<-res.none[res.none$n_markers>0,]
library(ggplot2)
...
p3
p4 <- ggplot(res.none, aes(n_markers, youden, group = group)) + geom_line() + geom_smooth(aes(group=1))
p4
...
plot(seq(10,150,5),y_prd_none, col='red', lwd=2)
plot(seq(10,150,5),y2_prd_none, col='red', lwd=2)
plot(seq(10,150,5),y3_prd_none, col='red', lwd=2)
plot(seq(10,150,5),y4_prd_none, col='red', lwd=2)
# ------------------------------
# ------- huberk --------
#-------------------------------
load(file = "ogclust_K3_1000genes_6groups_gamma1_delta3_outlier_huberk.rdata")
...
res.huber<-res.huber[!(res.huber$n_markers<50 & res.huber$ARI>0.35),]
...
p3
p4 <- ggplot(res.huber, aes(n_markers, youden, group = group)) + geom_line() + geom_smooth(aes(group=1))
p4
...
# ------- hubertf --------
#-------------------------------
load(file = "ogclust_K3_1000genes_6groups_gamma1_delta3_outlier_hubertf2.rdata")
...
res.adhuber<-data.frame(group=rep(1:(n.rep-2),each=nrow(tune_pars)),
                      n_markers=as.numeric(n_markers[,-c(5,8)]),
                      n_true_markers=as.numeric(n_true_markers[,-c(5,8)]), 
                      FPs=as.numeric(n_markers[,-c(5,8)])-as.numeric(n_true_markers[,-c(5,8)]),
                      FNs=15-as.numeric(n_true_markers[,-c(5,8)]),
                      perc_true_markers=as.numeric(n_true_markers[,-c(5,8)])/15,
                      specificity=(985-(as.numeric(n_markers[,-c(5,8)])-as.numeric(n_true_markers[,-c(5,8)])))/985,
                      ARI=as.numeric(ARI[,-c(5,8)]),
                      ARI_test=as.numeric(ARI_test[,-c(5,8)]),
                      R2=as.numeric(R2[,-c(5,8)]),
                      RMSE=as.numeric(RMSE[,-c(5,8)]),
                      R2_test=as.numeric(R2_test[,-c(5,8)]),
                      RMSE_test=as.numeric(RMSE_test[,-c(5,8)])
)
res.adhuber$youden<-with(res.adhuber, perc_true_markers+specificity-1)
res.adhuber<-res.adhuber[res.adhuber$n_markers>5,]
#res.adhuber<-res.adhuber[res.adhuber$n_markers<50 |res.adhuber$n_markers>100,]
library(ggplot2)
...
p3
p4 <- ggplot(res.adhuber, aes(n_markers, youden, group = group)) + geom_line() + geom_smooth(aes(group=1))
p4
...
# ------- median --------
#-------------------------------
load(file = "ogclust_K3_1000genes_6groups_gamma1_delta3_outlier_mediank.rdata")
...
df.outlier<-rbind(data.frame(measures="ARI",df1),
                    data.frame(measures="RMSE",df2),
                    data.frame(measures="R2",df3),
                    data.frame(measures="FPs",df4),
                    data.frame(measures="FNs",df5))
save(df.outlier, file="df.outlier3.rdata")
#library(gridExtra)
#install.packages("ggpubr")
library(ggpubr)
#grid.arrange(p1,p2,p3,p4,nrow=2)
pdf(file="smooth.plot.outlier2.pdf",width = 12, height=9)
