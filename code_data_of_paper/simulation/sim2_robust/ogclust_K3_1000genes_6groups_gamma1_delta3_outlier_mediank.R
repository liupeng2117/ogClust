...
gen.train<-gen.data.K3(n.rep, NG, n, theta, cluster_index, outlier_typeII = T)
...
par.est.all <- foreach(rep = 1:n.rep, .packages = c("mclust","OGClust")) %dopar% {
  ...
  for (l in 1:nrow(tune_pars)) {
    ...
    grp_assign_prd <- unlist(apply(w_est, 1, which.max))
    ARI <- tryCatch({
      adjustedRandIndex(grp_assign_prd, gen.train$grp_assign[rep, ])
    }, error = function(e) {
      NA
    })
    Y_prd <- tryCatch({
      miu_est[grp_assign_prd] + X1 * beta1_est + X2 * beta2_est
    }, error = function(e) {
      NA
    })
    ...
    grp_assign_prd_test<-unlist(apply(w_est_test,1,which.max))

    ARI_test<-tryCatch({
      adjustedRandIndex(grp_assign_prd_test, gen.test$grp_assign[rep, ])
      }, error = function(e) {
      NA
    })
    
    Y_prd_test <-tryCatch({
      miu_est[grp_assign_prd_test] + X1_test * beta1_est + X2_test * beta2_est
    }, error = function(e) {
      NA
    })
...
save(par.est.all, gen.train, gen.test, file = "ogclust_K3_1000genes_6groups_gamma1_delta3_outlier_mediank.rdata")
