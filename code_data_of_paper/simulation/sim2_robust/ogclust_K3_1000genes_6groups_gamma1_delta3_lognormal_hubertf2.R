...
#--------------------------------------------------#
K.all=3 #Set K = truth
lambda.all<-c(0.01,0.03,0.05,0.08,0.10,0.12,0.14,0.16,0.18,0.2) # penalty parameter # penalty parameter
tune_pars=cbind(K=rep(K.all,each=length(lambda.all)),lambda=rep(lambda.all,length(K.all)))

robust="hubertf2" # robust method
...
save(par.est.all, gen.train, gen.test, file = "ogclust_K3_1000genes_6groups_gamma1_delta3_lognormal_hubertf2.rdata")
