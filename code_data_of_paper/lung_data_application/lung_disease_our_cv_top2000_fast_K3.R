...
for(f in 1:length(flds)){
  ...
  res.all<-foreach(l=1:nrow(tune_pars)) %dopar% {
    ...
    for(i in 1:n.int){
        fit.res<-fit.mixture.model(n=n, K=K, np=np, NG=NG, lambda=lambda, alpha=0.5,
                                   df=df_train, G=G_train, Y=Y_train, X=X_train)
...
save.image(file="lung_disease_our_cv_top2000_fast_K3.Rdata")
...
for(i in 1:length(flds)){
  id=cvres.our1000[[i]]$fld_id
  Y_prd[id]<-cvres.our1000[[i]]$Y_prd_test
  grp_assign[id]<-cvres.our1000[[i]]$grp_assign_test
}
