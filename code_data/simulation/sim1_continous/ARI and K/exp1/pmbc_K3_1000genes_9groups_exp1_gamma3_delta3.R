...
save(res.pmbc,gen.res, file="pmbc_K3_1000genes_9groups_exp1_gamma3_delta3.rdata")
...
FPs<-c()
for(i in 1:10){
  TPs[i]<-sum(which(apply(abs(res.pmbc[[i]][[3]]$mu),1,mean)>=0.05) %in% 1:15)
  TNs[i]<-sum(which(apply(abs(res.pmbc[[i]][[3]]$mu),1,mean)<0.05) %in% 16:1000)
  FNs[i]<-sum(which(apply(abs(res.pmbc[[i]][[3]]$mu),1,mean)<0.05) %in% 1:15)
  FPs[i]<-sum(which(apply(abs(res.pmbc[[i]][[3]]$mu),1,mean)>=0.05) %in% 16:1000)
}
